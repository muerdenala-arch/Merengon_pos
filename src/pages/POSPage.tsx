...
export default function POSPage() {
  ...
  async function handleConfirmPayment(payment: Payment) {
    ...
    items.forEach((item) => {
      const branchIdForDeduction = item.deliveredFromBodega ? 'bodega' : currentBranchId!;
      adjustStock(item.product.id, branchIdForDeduction, -item.quantity);
      item.modifiers.toppings.forEach((t) => adjustToppingStock(t.id, branchIdForDeduction, -item.quantity));
      
      if (item.deliveredFromBodega) {
        // Registrar movimiento de stock en bodega
        api.stockMovements.create({
          ...
          type: 'SALE',
          notes: `Venta POS - Ticket (Pendiente sync)`, // We don't have ticketNumber yet, as it's generated by DB
          userId: currentUser.id,
          createdAt: new Date().toISOString(),
        }).catch(err => console.error('Error registrando movimiento de bodega:', err));
      }
    });
